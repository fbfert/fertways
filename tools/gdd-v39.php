...
/**
 * Gera o FERTWAYS GDD v39 (D-62, atualizado no D-141 e no D-160).
 *
 *     /usr/bin/php84 tools/gdd-v39.php > docs/FERTWAYS_GDD_v39_CONSOLIDADO.html
 *
 * Este arquivo é uma cópia evoluída de `gdd-v38.php` (que fica intocado, como o v36 e o v35 ficaram
 * intocados quando o v38 nasceu — D-141). Herda tudo o que estava atualizado até o D-140, e
 * acrescenta o que mudou desde então (D-141 a D-159, 19 decisões):
 *
 *  - **A zona neutra vira colmeia de 22 slots** — a mesma geometria da colônia, crescimento por
 *    nível (12 slots no 1, +1 por nível até 22 no 10), nível máximo de 5 para 10, e três estruturas
 *    repetíveis (D-144). É a mudança de regra mais funda desta leva: `§8.0`, novo.
 *  - **A periferia deixa de ser "qualquer lugar"** — só a célula que o Dono liberar é fundável, e a
 *    lista nasce vazia (D-147). Mexe no §1.1, que descrevia o contrário desde o D-51.
 *  - **A zona neutra deixa de nascer só nos 4 distritos** — o Dono cria zona em qualquer célula de
 *    periferia, escolhendo o mineral, e remove enquanto ela estiver livre (D-148).
 *  - **O mapa do painel de admin** — o planeta 101×101 inteiro, sem névoa, e as três ações que
 *    vivem nele: mover colônia, liberar fundação, criar zona (D-145 a D-148): `§11.6`, novo.
 *  - **O Frete do Governo leva vários recursos na mesma viagem** (D-151) — o teto de 30.000 sempre
 *    foi sobre a SOMA; era só a tela que mandava um recurso por vez.
 *  - **O card "Recursos por hora"** — a taxa nominal de produção e de consumo, separadas por
 *    recurso (D-153): `§3.4`, novo.
 *  - **O Reator de Energia vai até o nível 15** (D-157), pelas mesmas duas curvas do GDD, e as três
 *    trocas de slot do miolo (Depósito no centro, Reator no 6 — D-142, D-149/D-150, D-152), que
 *    entram sozinhas por serem lidas do banco.
 *
 * Ficaram de fora, de propósito, as decisões que só mudam tela e não mecânica — `/mapa` em tela
 * cheia com pinça de dois dedos e a legenda virando card (D-154 a D-156, D-158) e a Lista Mestra de
 * Assets de Estruturas (D-143) —, pelo mesmo critério que a seção 0 já publica.
 *
 * ---
 *
 ...
 * `federation_settings`, `silo_capacidades`), e boa parte tem **testes que provam que batem com o
 * GDD** (`tests/Gdd/GddSpecsTest`, `tests/Gdd/LogisticaSpecsTest`).
 *
...
<title>FERTWAYS — GDD v39 (consolidado)</title>
<style>
...
<header class="capa">
  <div class="env">
    <small>Game Design Document · versão 39 · consolidada</small>
    <h1>FERTWAYS</h1>
    <p>
      A base de produção do jogo. Este documento <b>substitui o v35</b> e, ao contrário dele,
      <b>não se contradiz</b>. É a terceira regeneração do v36 (D-62) — a primeira foi até o D-101,
      a segunda (v38) até o D-140; esta chega até o <b>D-159</b>, com a <b>zona neutra em colmeia
      de slots</b>, a <b>periferia curada célula a célula</b> e o <b>mapa do painel de admin</b>
      como frentes novas.
    </p>
...
<table>
  ...
  <tr>
    <td><b>Teto de estoque de outras construções</b></td>
    <td>O Tanque de Combustível já tem teto (D-131, §4.3); a Captação de Água e o "armazenamento"
        do §19.6 de outros recursos <b>continuam sem nenhum número publicado</b></td>
    <td>Nenhum outro recurso além de Biocombustível trava por capacidade de construção</td>
  </tr>
  <tr>
    <td><b>Níveis de veículo</b></td>
    <td><b>O que o nível muda.</b> O v35 publica custo até o nível 5 e nunca diz o que se ganha</td>
    <td>Só o nível 1 é vendido (dos dois tipos, desde o D-109). E as <b>duas tabelas de custo do Caminhão divergem</b> a partir do nível 2 <span class="d">D-37</span></td>
  </tr>
  <tr>
  ...
  <tr>
    <td><b>Guerra entre federações</b></td>
    <td>O 6º sub-ranking do §27.13 ("Guerras Vencidas — Federação", 10%) pressupõe uma guerra
        DECLARADA por federação; o jogo só tem combate entre DUAS COLÔNIAS</td>
    <td>O Ranking de Guerras soma 90%, não 100% — não renormalizado, de propósito (§8.11)</td>
  </tr>
</table>
...
<footer>
  <div class="env">
    <p>
      <b>FERTWAYS — GDD v39.</b> Substitui o v38 (D-141), que substituiu o v36 (D-62), que
      substituiu o v35 — os três ficam como registro histórico do que se pensava antes.
    </p>
    <p>
      As tabelas numéricas são geradas de <code>building_specs</code>, <code>resource_types</code>,
      <code>fabrica_veiculos</code>, <code>federation_settings</code> e <code>silo_capacidades</code> —
      as mesmas de onde o jogo lê — por <code>tools/gdd-v39.php</code>. O documento <b>não pode
      divergir do jogo</b>.
    </p>
    <p>
