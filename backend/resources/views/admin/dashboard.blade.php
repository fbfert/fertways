...
@section('content')
    ...
    <nav class="abas" style="background:transparent;padding:0;margin-bottom:16px">
        @foreach ($abas as $slug => $rotulo)
            <a href="{{ route('admin.dashboard', ['aba' => $slug]) }}"
               data-aba-dashboard="{{ $slug }}"
    ...
    {{--
        ...
        Aba própria — é o PRIMEIRO lugar onde se olha quando alguma coisa parece errada — "o que
        foi que mexeram?". Antes do D-61 esta pergunta não tinha resposta: o painel não deixava
        rastro nenhum.
    --}}
    @if ($aba === 'atos')
    ...
    @if ($aba === 'logistica')
        ...
        <div class="cartao">
            ...
            @endif
            @if ($obras->isEmpty() && $zonas->isEmpty())
                <p class="mut pequeno">Nada em obra e nenhuma zona ocupada agora.</p>
            @endif
        </div>
    @endif
