...
/**
 * A landing page + login/registro (pedido do usuário, 2026-07-17).
 *
 * Era só um cartão de formulário. Vira a porta de entrada do jogo: quem chega sem conta lê o que
 * o Fertways é antes de decidir, e quem já joga entra pelo mesmo lugar, mais rápido — o formulário
 * continua sendo exatamente o mesmo, só reposicionado.
 *
 * O conteúdo (headlines, pilares, passos) é reescrito a partir do deck em `/home/fertways/pitch` e
 * do GDD, mas só com o que o jogo **de fato entrega hoje** — nada de "em breve": não fabricar uma
 * promessa é a mesma regra de ouro que vale para o ledger e para o GDD.
 */
...
export function Login({ aoEntrar }: { aoEntrar: () => void }) {
  ...
  return (
    <div className="bg-sand min-h-screen">
      {/* ══════════════════════════════════════════════════════ topo + formulário */}
      <header className="border-rust/15 border-b px-6 py-4">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <Marca compacto />
          <a href="#entrar" onClick={irPara('login')} className="text-ink-soft hover:text-rust eyebrow">
            Entrar
          </a>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-6xl px-6 py-4">
        <div className="painel border-rust/15 overflow-hidden border">
          <img
            src="/pitch/hero.png"
            alt="Fertways: The Next Colony — construir, conectar, conquistar, transformar. Um mundo persistente onde economia, território, reputação e exploração contam a história dos jogadores."
            className="w-full"
            loading="lazy"
          />
        </div>
